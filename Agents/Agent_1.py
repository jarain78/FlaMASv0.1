...
class train_state(State):
    ...
    def deep_learning(self):
        ...
        local_weights, local_losses = federated_learning.train_local_model(AgName="pepita_hp_2", epoch=1)
    ...
    async def run(self):
        ...
        if (self.current_milli_time() - self.startMillis < waiting_time):
            print("-    END......")

        print(self.current_milli_time() - self.startMillis)

        self.deep_learning()
        # print(colored("-    Training Delta-T: ", str(timer() - value), 'red'))

        self.startMillis = self.current_milli_time()
...
if __name__ == "__main__":
    fd = Federated()
    jid_launch = "pepita_hp_1" + "@" + Config.xmpp_server
    passwd_launch = "test"
